...
const Title: FC<Props> = ({ title, classNames }): JSX.Element => {
   return (
      <div className={`relative text-ellipsis text-center ${classNames}`}>
         <span className="absolute right-0 -top-4 xl:-top-9 -z-10 w-full text-slate-900 opacity-80 select-none font-morabba text-4xl xl:text-8xl font-bold">{title}</span>
